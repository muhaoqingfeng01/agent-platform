...
public class TaskController {
    ...
    public Result<DagExecutionService.ExecutionStatusResponse> status(@Valid @RequestBody TaskExecutionGetRequest request) {
        return ResultRespHelper.responseInvoke("TaskController.status", request, (req) -> {
            TaskExecution execution = executionRepository.findByExecutionId(req.getExecutionId())
                    .orElseThrow(() -> new com.example.agent.common.exception.BusinessException(404,
                            "执行记录不存在: " + req.getExecutionId()));
            List<TaskStepExecution> steps = stepExecutionRepository.findByExecutionId(req.getExecutionId());
            return DagExecutionService.ExecutionStatusResponse.from(execution, steps);
        });
    }
