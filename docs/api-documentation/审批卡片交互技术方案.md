...
export const approvalService = {
  ...
  list: (filter: string = 'my-pending', page: number = 0, size: number = 20) =>
    apiClient.get<PageResponse<ApprovalWorkflow>>('/api/v1/approvals', {
      params: { filter, page, size },
    }),

  // 审批详情（含剩余时间）
  getById: (id: string) =>
    apiClient.get<ApprovalWorkflow>(`/api/v1/approvals/${id}`),

  // 同意审批
  approve: (id: string, comment?: string) =>
    apiClient.post(`/api/v1/approvals/${id}/approve`, { comment }),

  // 拒绝审批
  reject: (id: string, comment?: string) =>
    apiClient.post(`/api/v1/approvals/${id}/reject`, { comment }),

  // 获取待审批列表（断线重连用）
  listPending: () =>
    apiClient.get<ApprovalWorkflow[]>('/api/v1/approvals', {
      params: { filter: 'my-pending', size: 100 },
    }),

  // 审批统计
  getStats: () =>
    apiClient.get<ApprovalStats>('/api/v1/approvals/stats'),
};
```
...
export function ApprovalListPage() {
  ...
  return (
    <div>
      ...
      />
      <Table columns={columns} dataSource={data?.items} loading={isLoading} />
    </div>
  );
}
```
