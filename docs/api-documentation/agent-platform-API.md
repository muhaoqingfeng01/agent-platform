...
> ⚠️ **注意**: 本文档为自动生成的 API 参考。以下端点的权威定义以实际 Controller 代码为准：
> - **MessageController** (`POST /api/v1/conversations/messages/*`) — 5 个端点：send / stream / list / before / feedback
> - **InteractionController** (`/api/v1/interactions/*`) — 2 个端点：POST /execute（同步）/ GET /modes（模式查询）
> - SSE 流式端点统一通过 `InteractionApplicationService` → `InteractionStrategyFactory` 策略工厂路由
> - 所有端点使用 POST + Request Body 传参，不使用 `@PathVariable`
> - 已移除不存在的端点：更新标题、状态流转
...
**接口地址**:`/api/v1/conversations/messages/stream`
...
**接口地址**:`/api/v1/conversations/messages/before`
...
**接口地址**:`/api/v1/conversations/messages/send`
...
**接口地址**:`/api/v1/conversations/messages/feedback`
